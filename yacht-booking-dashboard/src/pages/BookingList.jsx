    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900">รายการจองทั้งหมด</h2>
                    <p className="text-slate-500 text-sm">จัดการการจองและตรวจสอบสถานะ ({dateFilteredBookings.length} รายการ)</p>
                </div>
                <button
            ...
            <div className="flex flex-col lg:flex-row gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="flex-1">
                    ...
                    <input
                        type="text"
                        placeholder="ชื่อ, เบอร์, อีเมล, Reward ID..."
                        value={search}
                ...
                {/* Yacht Filter */}
                <div className="sm:w-40">
                    <label className="block text-xs font-medium text-slate-500 mb-1">เรือ</label>
                    <select
                        value={yachtFilter}
                        onChange={e => setYachtFilter(e.target.value)}
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl"
                    >
                        <option value="ALL">ทุกลำ</option>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                                ...
                                <tr key={booking.id} className="hover:bg-slate-50/50">
                                    ...
                                    <td className="px-4 py-4 text-center">
                                        <button
                                            ...
                                            title={booking.emailSent ? 'ส่งแล้ว' : 'ยังไม่ส่ง'}
                                        >
                                            {booking.emailSent ? '🟢' : '⚪'}
                                        </button>
                                    </td>
                            ...
                            {dateFilteredBookings.length === 0 && (
                                <tr><td colSpan="7" className="px-6 py-12 text-center text-slate-400">ไม่พบข้อมูลการจอง</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
